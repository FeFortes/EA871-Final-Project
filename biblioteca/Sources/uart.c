 ...
 */
uint8_t initUART(uint8_t div4, uint8_t taxa, unsigned int baud_rate) {
	/*!
	 * Configura&ccedil;&atilde;o de SIM
         */
	//SIM_CLKDIV1 &= ~(uint32_t)(SIM_CLKDIV1_OUTDIV4(0b111));  ////< bus_clock = clock/1
	//SIM_CLKDIV1 |= (uint32_t)(SIM_CLKDIV1_OUTDIV4(div4));  ////< bus_clock = clock/[div4]
     ...
     */
	PORTA_PCR1 |= PORT_PCR_MUX(0b010);//encontrado no manual essa eh a alternativa 2 do mux         ///< Configure pino com fun&ccedil;&atilde;o UART0_RX
	PORTA_PCR2 |= PORT_PCR_MUX(0b010);//encontrado no manual essa eh a alternativa 2 do mux         ///< Configure pino com fun&ccedil;&atilde;o UART0_TX
	 ...
	 */
	UART0_C4 &= ~(UART0_C4_OSR(0x1C));           ///< Resetar o campo 
	UART0_C4 |= (UART0_C4_OSR(taxa-1));
	
	if (!((taxa-1) & 0x01)) 
			UART0_C4 &= ~(UART0_C4_OSR(0b00001));//ftp://ftp.dca.fee.unicamp.br/pub/docs/ea871/apostila_C/Operadores.pdf
	if (!((taxa-1) & 0x02))  
			UART0_C4 &= ~(UART0_C4_OSR(0b00010));//ftp://ftp.dca.fee.unicamp.br/pub/docs/ea871/apostila_C/Operadores.pdf
	 ...
	 */
	uint32_t SBR = (uint32_t)(20971520./(taxa * baud_rate));
	 ...
	 */
	UART0_BDH &= ~UART_BDH_SBR(0b11111);//<--  SBR bits mais significativos                           
	UART0_BDL &= ~UART_BDL_SBR(0b11111111);//<--  SBR bits menos significativos          
